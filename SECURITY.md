...
- **Identity is cryptographic, not network-based** -- a pod's identity derives
  ...
  not trust anchors.
- **Policy is deny-by-default when selected** -- when a WirescalePolicy or
  NetworkPolicy selects a pod, that pod defaults to deny for the selected
  direction. The Wirescale deployment guide RECOMMENDS installing a
  cluster-wide default-deny policy for full isolation.
...
Map 1: identity_cache (LPM trie, per-node)
  Key:   IP prefix (e.g., fd00:001d:0001:0003::7/128)
  Value: {
...
Pod A sends packet to Pod B:
  ...
  2. Node-A guarantees Pod A has IP fd00:001d:0001:0001::5
     (only kubelet on node-A can assign this IP)
  3. wirescale-agent resolves fd00:001d:0001:0001::5 = pod "web-xyz"
...
```json
{
  ...
  "details": {
    ...
    "allowed_ips": ["fd00:001d:0001:0003::/64"],
...
- **Mitigations:**
  ...
  - Agents can cross-verify: the IP prefix MUST belong to the node CIDR
    that the WireGuard peer is authorized for. If control says IP fd00:001d:0001:0003::7
