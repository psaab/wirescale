...
TC egress eBPF on host-side veth:
  ...
  - Deterministic mapping: 100.64.N.P -> fd00:001d:CCCC:N::P (CCCC = cluster)
  - If dst is another pod: dst 100.64.M.Q -> fd00:001d:CCCC:M::Q
...
eth0 (pod side) -> veth (host side)
  ...
  | src: fd00:001d:0001:0001::5, dst: 64:ff9b::5db8:d822
  v
Host routing table:
...
```
Pod A: send to fd00:001d:0001:0002::7
  |
  v
eth0 -> veth -> host routing
  |
  | route: fd00:001d:0001:0002::/64 dev wg0
  v
...
clat0 -> TC eBPF on veth:
  100.64.1.5 -> fd00:001d:0001:0001::5 (src)
  100.64.2.7 -> fd00:001d:0001:0002::7 (dst)  [deterministic, no NAT64 prefix!]
  |
  v
Host routing: fd00:001d:0001:0002::/64 dev wg0
  |
  v
...
Host routing -> veth
  ...
  | fd00:001d:0001:0001::5 -> 100.64.1.5 (src)
  | fd00:001d:0001:0002::7 -> 100.64.2.7 (dst)
  v
...
Global allocation (ULA or GUA):
  Fleet:    fd00:001d::/32      (ULA) or 3fff:1234::/32  (GUA)
    |
    +-- Cluster A: fd00:001d:0000::/48                    (/48 per cluster)
    |     +-- Host 1: fd00:001d:0000:0001::/64            (/64 per host)
    |     +-- Host 2: fd00:001d:0000:0002::/64
    |     +-- ...
    |     +-- Host N: fd00:001d:0000:N::/64
    |
    +-- Cluster B: fd00:001d:0001::/48
    |     +-- Host 1: fd00:001d:0001:0001::/64
    |     +-- ...
    |
    +-- Cluster C: fd00:001d:0002::/48
          +-- ...
```
...
fd00:001d:0000:0001::/64 dev wg0 peer <host-1-key>
fd00:001d:0000:0002::/64 dev wg0 peer <host-2-key>
...
fd00:001d:0000:2710::/64 dev wg0 peer <host-10000-key>

# Cross-cluster: one aggregate per remote cluster
fd00:001d:0001::/48 dev ws-resolve0 metric 100    # → Cluster B
fd00:001d:0002::/48 dev ws-resolve0 metric 100    # → Cluster C
fd00:001d:0003::/48 dev ws-resolve0 metric 100    # → Cluster D
...
```
...
1. Pod A sends to fd00:001d:Y:N::P (matches aggregate route fd00:001d:Y::/48)
   Agent eBPF intercepts on ws-resolve0: no WireGuard peer for /64  ~μs

2. Agent → local wirescale-control (Cluster X)
   "Resolve fd00:001d:Y:N::/64 -- which host, which cluster?"       ~5-10 ms
...
4. Controller (Cluster X) → Controller (Cluster Y)
   "Resolve fd00:001d:Y:N::/64 to host endpoint and public key"     ~10-30 ms
